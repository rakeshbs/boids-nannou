...
const VEHICLE_RECT_WIDTH: f32 = 30.0;
...
impl Model {
    pub fn update(&mut self, app: &App) {
        ...
        let mut quadtree: QuadTree = QuadTree::new(Rectangle::new(
            -half_width,
            -half_height,
            w_rect.w(),
            w_rect.h(),
...
fn view(app: &App, _model: &Model, frame: Frame) {
    ...
    //for v in found {
    //draw.ellipse().radius(4.0).xy(v).color(YELLOW);
    //}
    //draw.rect()
    ...
    //.stroke_weight(1.0)
    //.stroke_color(RED);
    //let points: Vec<Vec3> = _model
    //.vehicles
    //.iter()
    //.map(|v| Vec3::new(v.position.x, v.position.y, 0.0))
    //.collect();
    //draw.point_mode().mesh().points(points).color(WHITE);
